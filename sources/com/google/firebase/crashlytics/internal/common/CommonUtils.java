...
public class CommonUtils {
    ...
    public static String createInstanceIdFrom(String... strArr) {
        ...
        for (String str : strArr) {
            if (str != null) {
                arrayList.add(str.replace(PGTransHeader.CONNECTOR, "").toLowerCase(Locale.US));
            }
        }
    ...
    */
    ...
    public static int getDeviceState(Context context) {
        int i2 = isEmulator(context) ? 1 : 0;
        if (isRooted(context)) {
            i2 |= 2;
        }
        return isDebuggerAttached() ? i2 | 4 : i2;
    }
    ...
    public static String getResourcePackageName(Context context) {
        ...
        if (i2 > 0) {
            try {
                ...
                return TapjoyConstants.TJC_DEVICE_PLATFORM_TYPE.equals(resourcePackageName) ? context.getPackageName() : resourcePackageName;
                ...
                if (!TextUtils.isEmpty(extractFieldFromSystemFile)) {
                    ...
                    try {
                        if (upperCase.endsWith("KB")) {
                            j3 = convertMemInfoToBytes(upperCase, "KB", 1024);
                        } else if (upperCase.endsWith("MB")) {
                            j3 = convertMemInfoToBytes(upperCase, "MB", 1048576);
                        } else if (upperCase.endsWith("GB")) {
                            j3 = convertMemInfoToBytes(upperCase, "GB", 1073741824);
                        } else {
